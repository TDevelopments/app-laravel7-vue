  ...
  <div>
    <v-app class="color-background">
      <v-system-bar height="35" window color="#FBFBFB" class="p-page-new">
        ...
        <v-spacer></v-spacer>
        <v-btn small text class="text-capitalize hidden-xs-only">
          <v-icon v-if="isLoggedIn" color="#D22E2E">mdi-logout</v-icon>
          <v-icon v-else color="#D22E2E">mdi-login</v-icon>
          <div v-if="isLoggedIn" class="color-nav">Logout</div>
          <div v-else class="color-nav">Login/Register</div>
        </v-btn>
      </v-system-bar>
      ...
      <v-app-bar flat color="#fff" class="hidden-sm-and-up" max-height="64">
        <v-toolbar-title @click="homeRoute">
          <div class="d-flex">
            <v-img max-height="50" max-width="50" contain src="/images/LogoPNG.png"></v-img>
            <div class="my-auto ml-2 title-nav">BIZZPERU</div>
          </div>
