<template>
  <div>
    <v-col>
      <h3>Editar Orden</h3>
      <v-btn @click="updateOrder">Guardar</v-btn>
    </v-col>
    <v-divider></v-divider>
    <!-- <v-data-table
      v-model="selected"
...
export default {
  ...
  methods: {
    getOrder() {
      axios.get(`/api/v1/sale-orders/${this.$route.params.id}`).then(response => {
        this.p = response.data.data.Payment;
        this.stateOrder = response.data.data.StateOrder.id;
        this.payment = response.data.data.Payment == null ? {} : response.data.data.Payment;
        if (response.data.data.Payment != null) {
          this.payment.PaymentMethodId = response.data.data.Payment.sale_payment_method_id;
          this.payment.PaymentStatusId = response.data.data.Payment.sale_payment_status_id;
    ...
    createPayment(data) {
      axios
        .post(`/api/v1/sale-orders/${this.$route.params.id}/payment`, data)
        .then(response => {})
        .catch(error => {});
    },
    updatePayment(data) {
      axios
        .put(`/api/v1/sale-orders/${this.$route.params.id}/payment`, data)
        .then(response => {})
        .catch(error => {});
    },
    getStateOrder() {
      axios.get('/api/v1/sale-state-orders?list=true').then(response => {
    ...
    updateOrder() {
      ...
      let dat = {
        SaleStateOrderId: this.stateOrder,
      };
      axios({
        url: `/api/v1/sale-orders/${this.$route.params.id}`,
        method: 'PUT',
        data: dat,
      })
        .then(resp => {
