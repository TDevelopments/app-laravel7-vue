<template>
  <div>
    ...
    >
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title>Conceptos de Pago</v-toolbar-title>
          <v-divider class="mx-4" inset vertical></v-divider>
          <v-spacer></v-spacer>

          <v-btn color="#1B1B1B" dark @click="newConcept">
            Nuevo Concepto de Pago
          </v-btn>

          <v-dialog v-model="dialogDelete" max-width="290">
            <v-card>
              <v-card-title class="headline"> Eliminar Concepto de Pago </v-card-title>

              <v-card-text>
                ¿Estas seguro de eliminar esta <strong>concepto de pago</strong>?
              </v-card-text>
              <v-card-actions>
                <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    ...
    deleteItemConfirm() {
      axios
        .delete(`/api/v1/payment-concepts/${this.idDelete}`)
        .then(response => {
          this.getList();
          this.closeDelete();
        })
